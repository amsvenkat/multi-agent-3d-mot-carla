...
def build_dataset(cfg, transforms, dataset_catalog, is_train=True, val=False):
    '''
    Args:
    ...
    dataset_list = cfg.DATASETS.VALIDATION if val else dataset_list
    ...
    for dataset_name in dataset_list:
        ...
        # adding for validation script
        args["val"] = val
        # make dataset from factory object
...
def build_val_loader(cfg, is_train=False):
    num_gpus = get_world_size()
    images_per_batch = cfg.SOLVER.IMS_PER_BATCH
    assert images_per_batch % num_gpus == 0, \
            "SOLVER.IMS_PER_BATCH ({}) must be divisible by the number of GPUs ({}) used." \
    ...
    transforms = build_transforms(cfg, is_train)
    # changeds for validation script
    datasets = build_dataset(cfg, transforms, DatasetCatalog, is_train, val=True)

    data_loaders = []
    for dataset in datasets:
        sampler = samplers.InferenceSampler(len(dataset))
        batch_sampler = torch.utils.data.sampler.BatchSampler(
            sampler, 1, drop_last=True
        )
        collator = BatchCollator(cfg.DATALOADER.SIZE_DIVISIBILITY)
        num_workers = cfg.DATALOADER.NUM_WORKERS
        data_loader = torch.utils.data.DataLoader(
