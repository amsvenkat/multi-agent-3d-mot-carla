...
class KITTIDataset(Dataset):
    def __init__(self, cfg, root, is_train=True, transforms=None, val=False):
        super(KITTIDataset, self).__init__()
        # self.root = root
        # self.image_dir = os.path.join(root, "image_2")
        # self.label_dir = os.path.join(root, "label_2")
        self.root = root 
        #"/export/amsvenkat/project/ma_perception/data/train_jan/"
        self.image_dir = os.path.join(root, "image_3")
        self.label_dir = os.path.join(root, "label_3")
        self.custom_data = cfg.DATASETS.CUSTOM_DATA
        # removed and hardcoded for the custom data
        ...
        if val:
            self.split = cfg.DATASETS.VALIDATION_SPLIT
        self.val = val
    ...
    def __getitem__(self, idx):
        ...
        """
        flipped = False
        # changes made for validation (and not self.val)
        if (self.is_train) and (random.random() < self.flip_prob) and not self.val :
            flipped = True
        ...
        # changes made for validation (and not self.val)
        affine = False
        if (self.is_train) and (random.random() < self.aug_prob) and not self.val:
            affine = True
...
# # get camera intrinsic matrix K
        if self.custom_data:
            ...
            VIEW_FOV = 90
            K = np.identity(3)
            K[0, 2] = VIEW_WIDTH / 2.0
            K[1, 2] = VIEW_HEIGHT / 2.0
            K[0, 0] = K[1, 1] = VIEW_WIDTH / (2.0 * np.tan(VIEW_FOV * np.pi / 360.0))
        else :
