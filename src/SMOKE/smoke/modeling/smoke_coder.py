...
class SMOKECoder():
    ...
    def encode_box2d(self, K, rotys, dims, locs, img_size):
        ...
        #custom data
        box3d_image = torch.matmul(K, box3d.double())
        # kitti data
    ...
    def decode_location(self,
        ...
        # proj_points_img = float(proj_points_img)
        
        # for customized data
        locations = torch.matmul(Ks_inv, proj_points_img.double())
        # for kitti data
...
if __name__ == '__main__':
    ...
    K = torch.tensor([[721.54, 0., 631.44],
                      ...
                      [0, 0, 1]]).unsqueeze(0)
    # For customized data
    # x = 960 / (2.0 * np.tan(90 * np.pi / 360.0))
    # # 
    # K = torch.tensor([[x, 0., 480],
    #                   [0., x, 270],
    #                   [0, 0, 1]]).unsqueeze(0)
