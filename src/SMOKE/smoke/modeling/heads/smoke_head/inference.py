...
class PostProcessor(nn.Module):
    ...
    def prepare_targets(self, targets):
        ...
        # changed as part of the object detector in Carla for single image
        trans_mat = targets.get_field("trans_mat")
        K = targets.get_field("K")
        size = torch.tensor(targets.size)
    ...
    def forward(self, predictions, targets):
        ...
        result = torch.cat([
            clses.float(), pred_alphas.float(), box2d.float(), pred_dimensions.float(), pred_locations.float(), pred_rotys.float(), scores.float()
        ], dim=1)
