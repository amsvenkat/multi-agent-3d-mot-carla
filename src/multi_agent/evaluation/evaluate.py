...
sys.path.append("/export/amsvenkat/project/3d-multi-agent-tracking/src/multi_agent/AB3DMOT_libs/")

import mailpy
from AB3DMOT_libs.dist_metrics import iou, dist3d
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="/export/amsvenkat/project/data/seqs/", max_truncation = 0, min_height = 25, max_occlusion = 2, \
        mail=None, cls="car", eval_3diou=True, eval_2diou=False, num_hypo=1, thres=None, map_path =None, num_agent = 1 ):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)

        # changes made for Carla
        # filename_test_mapping = os.path.join(gt_path, "evaluate_tracking.seqmap_carla_cus.val")
        # filename_test_mapping =  ("evaluation/seqmap")
        filename_test_mapping =  map_path
        self.n_frames         = []
        self.sequence_name    = []
        ...
        self.gt_path           = os.path.join(gt_path, "gt_data")
        #self.gt_path           = "/export/amsvenkat/project/data/seqs_v2/seqs_1/gt_data/" #"/export/amsvenkat/project/3d-multi-agent-tracking/seqs/gt_data/"
        self.t_sha             = t_sha
        #self.t_path            = os.path.join("./results/KITTI", t_sha, "data_%d" % (int(num_hypo)-1))
        # self.t_path            = "/export/amsvenkat/project/data/seqs_v2/seqs_1/tracking_data/singleagent/"     #seqs/tracking_data/"
        if num_agent == "single":
            self.t_path           =  os.path.join(gt_path, "tracking_data/singleagent")
        else:
            self.t_path           =  os.path.join(gt_path, "tracking_data/multiagent")
    ...
    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False):
        ...
        for seq, s_name in enumerate(self.sequence_name):
            ...
            for line in f:
                ...
                # if t_data.track_id == -1:
                if t_data.track_id == -99 and t_data.obj_type.lower() == "dontcare":
                    # print(;)
                    continue
    ...
    def getThresholds(self, scores, num_gt, num_sample_pts=num_sample_pts):
        ...
        scores.sort()
        #print(scores.shape, num_gt, num_sample_pts)
        scores = scores[::-1]
        current_recall = 0
        thresholds = []
        recalls = []
        for i, score in enumerate(scores):
            ...
            if (((r_recall - current_recall) < (current_recall - l_recall)) and (i < (len(scores) - 1))):
                continue
            #print(i, score, current_recall)
            thresholds.append(round(score,2))
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
                ...
                for gg in g:
                    ...
                    for tt in t:
                        ...
                        if self.eval_2diou:
                            ...
                            elif self.cls=="pedestrian" :
                                c = 1 - iou(gg, tt, metric="iou_2d")
        ...
        if (self.fp+self.tp)==0 or (self.tp+self.fn)==0:
            ...
            # print("tp: {}, fp: {}, fn: {}, sum : {}".format(self.tp, self.fp, self.fn, self.tp+self.fp+self.fn))
        ...
        if self.tp==0:
            ...
            # print("total cost: {}, tp: {}".format(self.total_cost, self.tp))
...
class stat:
    ...
    def plot(self):
        ...
        #save_dir = "/export/amsvenkat/project/3d-multi-agent-tracking/data/seqs/plot/"
        save_dir = os.path.join("/export/amsvenkat/project/3d-multi-agent-tracking/results/", self.t_sha)
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, agents, map_path, gt_path):
    """
    ...
    for c in ["car", "pedestrian"]: #("cyclist", "pedestrian", "car"):
        print(c)
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,eval_3diou=eval_3diou,eval_2diou=eval_2diou,num_hypo=num_hypo,thres=thres, map_path=map_path, num_agent = agents, gt_path = gt_path)
        print(e)
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>=5:
        if sys.argv[3] == "2D":
            ...
            print("Usage: python3 scripts/KITTI/evaluate.py result_sha num_hypothesis(e.g., 1) dimension(e.g., 2D or 3D) thres(e.g., 0.25)")
            sys.exit(1);    
        if len(sys.argv)==6: thres = float(sys.argv[4])
        else: thres = None

        num_agent = sys.argv[5]
    else:
        eval_3diou, eval_2diou = True, False        # eval 3d
        thres = None
    
    map_path = "evaluation/seqmap_5_v1"
    gt_path = "/export/amsvenkat/project/data/seqs/"
    # evaluate results
    success = evaluate(result_sha,mail,num_hypo,eval_3diou,eval_2diou,thres, num_agent, map_path, gt_path)