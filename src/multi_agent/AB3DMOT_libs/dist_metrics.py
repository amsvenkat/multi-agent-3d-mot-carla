...
def compute_height_carla(box_a, box_b, inter=True):

	boxa_corners, boxa_bot = Merger.construct_3d_box_carla(box_a, detector=False)
	boxb_corners, boxb_bot = Merger.construct_3d_box_carla(box_b, detector=False)
...
def compute_bottom(box_a, box_b):
	...
	# everything in world coordinates (x, y, z)
	corners1 = Box3D.box2corners3d_camcoord(box_a) 	# 8 x 3
	corners2 = Box3D.box2corners3d_camcoord(box_b)	# 8 x 3
...
def iou(box_a, box_b, metric='giou_3d'):
	...
	#print(box_a)
	boxa_bot, boxb_bot = boxa_bot[:, :2], boxb_bot[:, :2]
	...
	if '2d' in metric:		 	# return 2D IoU/GIoU
		...
		# AV - changed from compute_height to compute_height_carla
		...
		if metric == 'giou_3d':
			...
			#print('I2d, c_2d,  i3d, c3d, u3d', I_2D, C_2D, I_3D, C_3D, U_3D)
